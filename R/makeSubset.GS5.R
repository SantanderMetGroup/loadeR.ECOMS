...
#' applying the java methods makeSubset and readDataSlice. Subroutine of loadSeasonalForecast.GS5
#' 
...
#' @author J Bedia 
#' 
makeSubset.GS5 <- function(grid, latLon, runTimePars, foreTimePars) {
      message("[", Sys.time(), "] Retrieving data subset ..." )
      gcs <- grid$getCoordinateSystem()
      dimNames <- rev(names(scanVarDimensions(grid))) # reversed!
      z <- .jnull()
      memberRangeList <- runTimePars$memberRangeList
      ...
      for (i in 1:length(memberRangeList)) {
            ...
            for (j in 1:length(runTimePars$runTimeRanges[[i]])) {
                  ...
                  aux.list2 <- NULL
                  # Deaccumulator
                  if (isTRUE(foreTimePars$deaccumFromFirst)) {
