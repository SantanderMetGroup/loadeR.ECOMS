...
getVerticalLevelPars.ECOMS <- function(grid, level) {
    gcs <- grid$getCoordinateSystem()
    if (gcs$hasVerticalAxis()) {
        if (is.null(level)) {
            stop("Variable with vertical levels: '@level' following the variable name is required.\nGo to <http://www.meteo.unican.es/ecoms-udg/DataServer/ListOfVariables> for details")
        }
        levelInd <- gcs$getVerticalAxis()$findCoordElement(level)
        if (levelInd < 0) {
            stop("Vertical level not found.\nGo to <http://www.meteo.unican.es/ecoms-udg/DataServer/ListOfVariables> for valid vertical level values")
        }
